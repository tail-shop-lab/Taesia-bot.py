...
class Teasia(commands.Bot):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith("") and message.author.id != 766932365426819092:
            ...
            level = cur.fetchone()
            if level == None:
                cur.execute("INSERT INTO level VALUES (?, ?, ?, ?, ?, ?)", (message.author.id, 0, 1, 0, message.guild.id, message.author.display_name))
                conn.commit()
            exp = [100, 220, 350, 480, 610, 740, 870, 1100, 1230, 1360, 1490, 1620, 1750, 2000, 2500, 3000, 3500, 4000, 6000, 8000, 10000]
            await asyncio.sleep(0.3)
            cur.execute(f"UPDATE level SET exp = exp + 1 WHERE user = {message.author.id} AND guild_id = {message.guild.id}")
            conn.commit()
            cur.execute(f"SELECT * FROM level WHERE user = {message.author.id} AND guild_id = {message.guild.id}")
            level2 = cur.fetchone()
            if level2[3] == 1:
                ...
                try:
                    cur.execute(f"UPDATE level SET lv = lv + 1 WHERE user = {message.author.id} AND guild_id = {message.guild.id}")
                    conn.commit()
                    embed = discord.Embed(title="레벨업!🆙", colour=discord.Colour.gold())
                    embed.add_field(name="축하드립니다!",
                                    value="현재레벨:" + str(level2[2]) + ".Lv" "\n현재경험지:" + str(
                                        level2[1]) + ".exp" + "\n다음레벨업까지:" + str(int(exp[level2[2]]) - int(level2[1])) + ".exp 이상필요해요!")
                    embed.set_footer(text="아나타...좀 고여가시는군요?")
                    await message.channel.send(f"{message.author.mention}", embed=embed)
                except IndexError:
                    cur.execute(f"UPDATE level SET lv = lv + 1 WHERE user = {message.author.id} AND guild_id = {message.guild.id}")
                    cur.execute(f"UPDATE level SET max = 1 WHERE user = {message.author.id} AND guild_id = {message.guild.id}")
                    conn.commit()
                    embed = discord.Embed(title="레벨업!🆙", colour=discord.Colour.gold())
                    embed.add_field(name="축하드립니다!",
                                    value="현재레벨:" + str(level2[2]) + ".Lv MAX!🏆" "\n현재경험지:" + str(
                                        level2[1]) + ".exp MAX!🏆" + "\n~~다음레벨업까지~~: 최고레벨달성!🏆")
                    embed.set_footer(text="아나타...고이다 못해 썩었군요?")
                    await message.channel.send(f"{message.author.mention}님! 최고레벨을 달성하셨어요!🎉🎉", embed=embed)
        ...
        if message.content.startswith('ㅌ영화순위'):
            ...
            embed = discord.Embed(title="영화 순위", description="영화 순위 입니다", colour=0x00f000).set_footer(
