...
class admin(commands.Cog):
    ...
    async def user_info(self, ctx, member: discord.Member):
        ...
        try:
            ...
            check_mute = "진행 중인 처벌이 없습니다."
        date = datetime.datetime.utcfromtimestamp(((int(member.id) >> 22) + 1420070400000) / 1000)
        embed = discord.Embed(title=f"{member.mention}님의 정보", color=0x00ff00,
                              timestamp=datetime.datetime.now())
        embed.add_field(name="이름", value=member.name, inline=True)
        embed.add_field(name="서버 닉네임", value=member.display_name, inline=True)
        ...
        embed.add_field(name="받고 있는 처벌", value=f"```{check_mute}```", inline=False)
    ...
    async def broadcast(self, ctx, channel: discord.TextChannel, *, title="알림"):
        ...
        try:
            ...
            embed = discord.Embed(title=title, description=f"```{msg.content}```", colour=discord.Colour.blue(),
                                  timestamp=datetime.datetime.now())
            embed.set_footer(text=f'{ctx.author.mention} | 알림을 확인하셨다면 아래 반응에 응답해주세요!', icon_url=ctx.author.avatar_url)
    ...
    async def clear(self, ctx, amount: int = 5):
        ...
        await msg.edit(content="> :wastebasket: 청소 준비중..")
        time.sleep(2)
        await msg.edit(content="> :wastebasket: 청소 중..")
        time.sleep(2)
        await msg.edit(content="> :wastebasket: 청소 끝!")
        time.sleep(1)
    ...
    async def warn(self, ctx, member: discord.Member, *, reason="사유 미작성"):
        ...
        embed = discord.Embed(title="경고장 발급됨",
                              colour=discord.Colour.red(),
                              timestamp=datetime.datetime.now())
        embed.add_field(name="처벌 내용", value="```경고```", inline=False)
        embed.add_field(name="유저", value=member.mention)
        embed.add_field(name="사유", value=f"```{reason}```", inline=False)
        embed.set_footer(text="경고 5회 누적 시 밴 처리되오니 주의바랍니다.")
        await ctx.send(embed=embed, delete_after=20)

        if check == 5:
            await member.ban(reason="경고 5회 누적")
    ...
    async def mute(self, ctx, member: discord.Member, time: str, *, reason):
        ...
        await self.app.get_channel(749998827356029008).send(embed=embed)
        await self.app.get_channel(730016508859645992).send(embed=embed)
        await ctx.send(f"> :ballot_box_with_check: {member.mention}님의 입을 일시적으로 막았습니다", delete_after=3)
    ...
    async def reboot(self, ctx):
        await ctx.send("후에엥 ㅜㅜ")
        await ctx.send("리붓하고올게요... ㅜㅜ")
